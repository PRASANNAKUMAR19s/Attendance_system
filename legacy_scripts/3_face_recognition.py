...
def init_attendance_file(path):
    if not os.path.isfile(path):
        with open(path, "w", newline="") as f:
            writer = csv.writer(f)
            writer.writerow(["RegNo", "Name", "Date", "Period", "MarkedTime", "Status"])
...
def load_student_map():
    ...
    with open(NAME_MAP, newline="") as f:
        reader = csv.DictReader(f)
        for row in reader:
            mapping[int(row["ID"])] = {"reg_no": row["ID"], "name": row["Name"]}
    return mapping
...
def start_recognition():
    ...
    while True:
        ...
        if last_period_name and (period is None or period["name"] != last_period_name):
        ...
        if period is None:
            ...
            cv2.putText(
                frame, msg, (10, 50), cv2.FONT_HERSHEY_SIMPLEX, 0.9, (0, 255, 255), 2
            )
                ...
                if already:
                    ...
                    with open(attendance_path, newline="") as f:
                        for row in csv.DictReader(f):
                            if (
                                row["RegNo"] == str(reg_no)
                                and row["Period"] == period["name"]
                            ):
